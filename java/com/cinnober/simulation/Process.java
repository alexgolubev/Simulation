...
package com.cinnober.simulation;
...
*   
* @see dk.ruc.javaSimulation.Link
* @see dk.ruc.javaSimulation.Head
*/

public abstract class Process extends Link {
    
    public static final int cProcessInit = 0;
    public static final int cProcessDone = -1;
    public static final int cProcessError = -2;
    ...
    private static Set cProcessSet = Collections.synchronizedSet(new HashSet());
    @SuppressWarnings("unused")
    private static boolean cTermination;
    ...
    */
    ...
    static { 
        SQS = new Process() {
            protected int actions() { return 0; }
        };
    ...
    */
    private final void resumeCurrent() {
        if (MAIN == null)
        {
            MAIN = SQS.SUC;
            Process tCurrent;
            ...
            {
                tCurrent = SQS.SUC;
                //System.out.println("" + time() + " " + tCurrent.getClass().getSimpleName() + " " + mState );
                tCurrent.mState = tState = tCurrent.actions();
                if (tState == cProcessDone)
                {
                    // Terminated
                    tCurrent.TERMINATED = true;
                    if (tCurrent == MAIN)
                    {
                        while (SQS.SUC != SQS)
                        {
                        ...
                        // Allready canceled MAIN.cancel();
                        return;
                    }
                    else
                    {
                        if (tCurrent.SUC != null)
                        {
                            tCurrent.cancel();
                        }
                        tCurrent.mAction = 1;
                ...
                else if (tState == cProcessError)
                {
                    throw new TerminateException();
                }
                if (tCurrent != SQS.SUC)
                {
                    if (tCurrent.mAction == 1)
                    {
                        tCurrent = SQS.SUC;
                        tCurrent.mAction = 0;
                    }
                    ...
                    {
                        // mAction = count of hold/wait/resume 
                        System.out.println("Action = " + tCurrent.mAction);
                    }
                }
            }
        }
        else
        {
            if (++mAction >= 2)
            {
                System.out.println("Multiple holds for state " + mAction +
                        " " + this.getClass().getSimpleName() + " " + mState);
            }
        }
    } 
}
